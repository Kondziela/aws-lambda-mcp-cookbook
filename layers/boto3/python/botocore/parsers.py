...
class ResponseParser:
    ...
    def _has_unknown_tagged_union_member(self, shape, value):
        if shape.is_tagged_union:
            ...
            serialized_member_names = [shape.members[member].serialization.get('name', member) for member in shape.members]
            if tag not in serialized_member_names:
                LOG.info(
                    'Received a tagged union response with member unknown to client: %s. Please upgrade SDK for full response support.',
                    tag,
                )
                return True
        return False
...
class BaseXMLResponseParser(ResponseParser):
    ...
    def _parse_xml_string_to_dom(self, xml_string):
        try:
            ...
            raise ResponseParserError(f'Unable to parse response ({e}), invalid XML received. Further retries may succeed:\n{xml_string}')
        return root
...
class BaseCBORParser(ResponseParser):
    ...
    def parse_data_item(self, stream):
        ...
        if major_type in self.major_type_to_parsing_method_map:
            ...
            raise ResponseParserError(f'Unsupported inital byte found for data item- Major type:{major_type}, Additional info: {additional_info}')
    ...
    def _parse_unsigned_integer(self, stream, additional_info):
        ...
        if additional_info < 24:
            ...
            raise ResponseParserError(
                f'Invalid CBOR integer returned from the service; unparsable additional info found for major type 0 or 1: {additional_info}'
            )
    ...
    def _read_from_stream(self, stream, num_bytes):
        ...
        if len(value) != num_bytes:
            raise ResponseParserError('End of stream reached; this indicates a malformed CBOR response from the server or an issue in botocore')
        return value

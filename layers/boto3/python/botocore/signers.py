...
def add_dsql_generate_db_auth_token_methods(class_attributes, **kwargs):
    ...
    class_attributes['generate_db_connect_admin_auth_token'] = dsql_generate_db_connect_admin_auth_token
...
def _dsql_generate_db_auth_token(self, Hostname, Action, Region=None, ExpiresIn=900):
    ...
    if Action not in possible_actions:
        raise ParamValidationError(report=f'Received {Action} for action but expected one of: {", ".join(possible_actions)}')

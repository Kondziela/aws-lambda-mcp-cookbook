...
class ClientCreator:
    ...
    def create_client(
        ...
        try:
            ...
            partition_data = None
            logger.info(
                'No endpoints ruleset found for service %s, falling back to legacy endpoint routing.',
                service_name,
            )
    ...
    def _set_importexport_signature_version(self, client_meta, client_config, scoped_config):
        ...
        # if none was manually set. If a customer manually sets the signature
        # version, we always want to use what they set.
        configured_signature_version = _get_configured_signature_version('importexport', client_config, scoped_config)
...
class BaseClient:
    ...
    def __init__(
        ...
        if self._user_agent_creator is None:
            self._user_agent_creator = UserAgentString.from_environment().with_client_config(self._client_config)
    ...
    def _make_api_call(self, operation_name, api_params):
        ...
        if event_response is not None:
            ...
            http, parsed_response = self._make_request(operation_model, request_dict, request_context)
...
def _get_configured_signature_version(service_name, client_config, scoped_config):
    ...
    if scoped_config is not None:
        ...
        if service_config is not None and isinstance(service_config, dict):
            ...
            if version:
                logger.debug(
                    'Switching signature version for service %s to version %s based on config file override.',
                    service_name,
                    version,
                )
                return version

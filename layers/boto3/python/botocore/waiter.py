...
def create_waiter_with_client(waiter_name, waiter_model, client):
    ...
    waiter_class_name = str(f'{get_service_module_name(client.meta.service_model)}.Waiter.{waiter_name}')
    ...
    def _verify_supported_version(self, version):
        if version != self.SUPPORTED_VERSION:
            raise WaiterConfigError(
                error_msg=(
                    f'Unsupported waiter version, supported version must be: {self.SUPPORTED_VERSION}, but version of waiter config is: {version}'
                )
            )

...
class ClientArgsCreator:
    ...
    def compute_client_args(
        ...
        if client_config is not None:
            config_kwargs.update(
                ...
                request_min_compression_size_bytes=(client_config.request_min_compression_size_bytes),
    ...
    def _compute_inject_host_prefix(self, client_config, config_kwargs):
        ...
        if client_config is None or client_config._inject_host_prefix == 'UNSET':
            configured_disable_host_prefix_injection = self._config_store.get_config_variable('disable_host_prefix_injection')
    ...
    def compute_endpoint_resolver_builtin_defaults(
        ...
        return {
            ...
            EPRBuiltins.AWS_S3_USE_GLOBAL_ENDPOINT: (self._should_force_s3_global(region_name, s3_config)),
            ...
            EPRBuiltins.AWS_S3_DISABLE_MRAP: s3_config.get('s3_disable_multiregion_access_points', False),
            EPRBuiltins.SDK_ENDPOINT: given_endpoint,
            EPRBuiltins.ACCOUNT_ID: credentials.get_deferred_property('account_id') if credentials else None,
    ...
    def _compute_user_agent_appid_config(self, config_kwargs):
        ...
        if user_agent_appid is not None and len(user_agent_appid) > USERAGENT_APPID_MAXLEN:
            logger.warning(
                'The configured value for user_agent_appid exceeds the maximum length of %d characters.',
    ...
    def _compute_sigv4a_signing_region_set_config(self, config_kwargs):
        ...
        if sigv4a_signing_region_set is None:
            sigv4a_signing_region_set = self._config_store.get_config_variable('sigv4a_signing_region_set')
    ...
    def _compute_auth_scheme_preference_config(self, client_config, config_kwargs):
        ...
        if not isinstance(value, str):
            raise botocore.exceptions.InvalidConfigError(
                error_msg=(f'{config_key} must be a comma-delimited string. Received {type(value)} instead: {value}.')
            )

        value = ','.join(item.replace(' ', '').replace('\t', '') for item in value.split(',') if item.strip())

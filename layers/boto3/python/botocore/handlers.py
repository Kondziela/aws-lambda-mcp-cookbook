...
def check_for_200_error(response, **kwargs):
    ...
    if _looks_like_special_case_error(http_response.status_code, http_response.content):
        logger.debug(
            'Error found for response with 200 status code, errors: %s, changing status code to 500.',
            parsed,
        )
...
def document_base64_encoding(param):
    description = '**This value will be base64 encoded automatically. Do not base64 encode this value prior to performing the operation.**'
...
def validate_ascii_metadata(params, **kwargs):
    ...
    for key, value in metadata.items():
        try:
            ...
            error_msg = (
                f'Non ascii characters found in S3 metadata for key "{key}", value: "{value}".  \nS3 metadata can only contain ASCII characters. '
            )
            raise ParamValidationError(report=error_msg)
    ...
    if _has_expires_shape(operation_model.output_shape):
        if expires_value := response_dict.get('headers', {}).get('Expires'):
            ...
            try:
                ...
                logger.warning(
                    'Failed to parse the "Expires" member as a timestamp: %s. The unparsed value is available in the response under "ExpiresString".',
                    expires_value,
                )
...
def _has_expires_shape(shape):
    if not shape:
        return False
    return any(member_shape.name == 'Expires' and member_shape.serialization.get('name') == 'Expires' for member_shape in shape.members.values())
...
def _update_status_code(response, **kwargs):
    ...
    if response is None:
        return
    http_response, parsed = response
    parsed_status_code = parsed.get('ResponseMetadata', {}).get('HTTPStatusCode', http_response.status_code)
...
def _set_auth_scheme_preference_signer(context, signing_name, **kwargs):
    ...
    signature_version_set_in_code = isinstance(signature_version, ClientConfigString) or signature_version is botocore.UNSIGNED

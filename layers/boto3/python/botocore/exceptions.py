...
class ConnectionClosedError(HTTPClientError):
    fmt = 'Connection was closed before we received a valid response from endpoint URL: "{endpoint_url}".'
...
class ClientError(Exception):
    MSG_TEMPLATE = 'An error occurred ({error_code}) when calling the {operation_name} operation{retry_info}: {error_message}'
...
class InvalidS3AddressingStyleError(BotoCoreError):
    ...
    fmt = "S3 addressing style {s3_addressing_style} is invalid. Valid options are: 'auto', 'virtual', and 'path'"
...
class InvalidSTSRegionalEndpointsConfigError(BotoCoreError):
    ...
    fmt = 'STS regional endpoints option {sts_regional_endpoints_config} is invalid. Valid options are: "legacy", "regional"'
...
class MissingServiceIdError(UndefinedModelAttributeError):
    fmt = 'The model being used for the service {service_name} is missing the serviceId metadata property, which is required.'
...
class InvalidChecksumConfigError(BotoCoreError):
    ...
    fmt = 'Unsupported configuration value for {config_key}. Expected one of {valid_options} but got {config_value}.'


class UnsupportedServiceProtocolsError(BotoCoreError):
    """Error when a service does not use any protocol supported by botocore."""

    fmt = 'Botocore supports {botocore_supported_protocols}, but service {service} only supports {service_supported_protocols}.'

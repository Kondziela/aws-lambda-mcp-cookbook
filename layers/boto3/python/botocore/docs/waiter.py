...
def document_wait_method(
    ...
    waiter_config_members['Delay'] = DocumentedShape(
        name='Delay',
        type_name='integer',
        documentation=(f'<p>The amount of time in seconds to wait between attempts. Default: {waiter_model.delay}</p>'),
    )

    waiter_config_members['MaxAttempts'] = DocumentedShape(
        name='MaxAttempts',
        type_name='integer',
        documentation=(f'<p>The maximum number of attempts to be made. Default: {waiter_model.max_attempts}</p>'),
    )
    ...
    botocore_waiter_params = [
        DocumentedShape(
            name='WaiterConfig',
            type_name='structure',
            documentation=('<p>A dictionary that provides parameters to control waiting behavior.</p>'),

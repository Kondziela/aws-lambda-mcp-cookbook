...
def resolve_auth_scheme_preference(preference_list, auth_options):
    ...
    unsupported = [scheme for scheme in preference_list if scheme not in AUTH_PREF_TO_SIGNATURE_VERSION]
    if unsupported:
        logger.debug('Unsupported auth schemes in preference list: %r', unsupported)

    combined = preference_list + service_supported
    prioritized_schemes = [scheme for scheme in dict.fromkeys(combined) if scheme in service_supported]
...
AUTH_PREF_TO_SIGNATURE_VERSION = {auth_scheme.split('#')[-1]: sig_version for auth_scheme, sig_version in AUTH_TYPE_TO_SIGNATURE_VERSION.items()}

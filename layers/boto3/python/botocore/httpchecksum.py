...
def resolve_request_checksum_algorithm(
    ...
    request_checksum_required = operation_model.http_checksum_required or http_checksum.get('requestChecksumRequired')
    ...
    if algorithm_member and algorithm_member in params:
        ...
        if algorithm_name not in supported_algorithms:
            ...
            raise FlexibleChecksumError(error_msg=f'Unsupported checksum algorithm: {algorithm_name}')
    elif request_checksum_required or (algorithm_member and request_checksum_calculation == 'when_supported'):
...
def apply_request_checksum(request):
    ...
    if algorithm == 'conditional-md5':
        ...
        raise FlexibleChecksumError(error_msg='Unknown checksum variant: {}'.format(algorithm['in']))
...
def _apply_request_trailer_checksum(request):
    ...
    if content_length is not None:
        ...
        if 'Content-Length' in headers:
            ...
            logger.debug("Removing the Content-Length header since 'chunked' is specified for Transfer-Encoding.")

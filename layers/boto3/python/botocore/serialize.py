    ...
    def _expand_host_prefix(self, parameters, operation_model):
        ...
        if bad_labels:
            raise ParamValidationError(
                report=(f'Invalid value for parameter(s): {", ".join(bad_labels)}. Must contain only alphanumeric characters, hyphen, or period.')
            )
...
class JSONSerializer(Serializer):
    ...
    def _serialize_type_timestamp(self, serialized, value, shape, key):
        serialized[key] = self._convert_timestamp_to_str(value, shape.serialization.get('timestampFormat'))
...
class CBORSerializer(Serializer):
    ...
    def _serialize_data_item(self, serialized, value, shape, key=None):
        ...
        if method is None:
            raise ValueError(f'Unrecognized C2J type: {shape.type_name}, unable to serialize request')
        method(serialized, value, shape, key)
    ...
    def _serialize_type_float(self, serialized, value, shape, key):
        if self._is_special_number(value):
            serialized.extend(self._get_bytes_for_special_numbers(value))  # Handle special values like NaN or Infinity
        else:
    ...
    def _partition_parameters(self, partitioned, param_name, param_value, shape_members):
        ...
        if location == 'uri':
            ...
            if isinstance(param_value, dict):
                ...
                new_param = [self._get_uri_and_query_string_value(value, member.member) for value in param_value]
    ...
    def _convert_header_value(self, shape, value):
        if shape.type_name == 'timestamp':
            ...
            if shape.member.type_name == 'string':
                converted_value = [self._escape_header_list_string(v) for v in value if v is not None]
            else:
...
class RestXMLSerializer(BaseRestSerializer):
    ...
    def _serialize_type_map(self, xmlnode, params, shape, name):
        ...
        for key, value in params.items():
            sub_node = ElementTree.SubElement(xmlnode, name) if self._is_shape_flattened(shape) else ElementTree.SubElement(node, 'entry')
    ...
    def _serialize_type_timestamp(self, xmlnode, params, shape, name):
        node = ElementTree.SubElement(xmlnode, name)
        node.text = str(self._convert_timestamp_to_str(params, shape.serialization.get('timestampFormat')))
        self._add_xml_namespace(shape, node)

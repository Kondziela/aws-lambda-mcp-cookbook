...
class UserAgentComponentSizeConfig:
    ...
    def _validate_input(self):
        if self.max_size_in_bytes < 1:
            raise ValueError(f'Invalid `max_size_in_bytes`: {self.max_size_in_bytes}. Value must be a positive integer.')
...
class UserAgentString:
    ...
    def _build_sdk_metadata(self):
        ...
        if (
            ...
            and (self._session_user_agent_name != _USERAGENT_SDK_NAME or self._session_user_agent_version != botocore_version)
        ):
            sdk_md.extend(
                [
                    UserAgentComponent(

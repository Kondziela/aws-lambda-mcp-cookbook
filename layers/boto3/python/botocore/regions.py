...
class EndpointResolver(BaseEndpointResolver):
        ...
        for partition in self._endpoint_data['partitions']:
            ...
            for endpoint_name in service_endpoints:
                ...
                if endpoint_variant_tags and is_regional_endpoint:
                    variant_data = self._retrieve_variant_data(service_endpoints[endpoint_name], endpoint_variant_tags)
                    if variant_data:
    ...
    def construct_endpoint(
        ...
        for partition in self._endpoint_data['partitions']:
            if use_dualstack_endpoint and (partition['partition'] in self._UNSUPPORTED_DUALSTACK_PARTITIONS):
                continue
            result = self._endpoint_for_partition(
                partition,
                service_name,
    ...
    def _endpoint_for_partition(
        ...
        if use_dualstack_endpoint and partition_name in self._UNSUPPORTED_DUALSTACK_PARTITIONS:
            error_msg = f'Dualstack endpoints are currently not supported for {partition_name} partition'
    ...
    def _resolve(
        ...
        if tags:
            result = self._resolve_variant(tags, endpoint_data, service_defaults, partition_defaults)
            if result == {}:
...
class EndpointRulesetResolver:
    ...
    def construct_endpoint(
        ...
        if not self._use_ssl and provider_result.url.startswith('https://') and 'Endpoint' not in provider_params:
    ...
    def _resolve_param_from_context(self, param_name, operation_model, call_args):
        ...
        if dynamic is not None:
            return dynamic
        operation_context_params = self._resolve_param_as_operation_context_param(param_name, operation_model, call_args)
    ...
    def auth_schemes_to_signing_ctx(self, auth_schemes):
        ...
        LOG.debug(
            'Selecting from endpoint provider\'s list of auth schemes: %s. User selected auth scheme is: "%s"',
        ...
        if self._requested_auth_scheme is not None:
            try:
                ...
                name, scheme = next(
                    ...
                    if self._does_botocore_authname_match_ruleset_authname(self._requested_auth_scheme, s['name'])
                )
            ...
            try:
                ...
                if fixable_with_crt:
                    raise MissingDependencyException(
                        msg='This operation requires an additional dependency. Use pip install botocore[crt] before proceeding.'
                    )
                else:

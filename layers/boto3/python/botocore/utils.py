...
class IMDSRegionProvider:
    ...
    def _create_fetcher(self):
        ...
        imds_config = {
            ...
            'ec2_metadata_v1_disabled': self._session.get_config_variable('ec2_metadata_v1_disabled'),
        }
...
class S3RegionRedirectorv2:
    ...
    def redirect_from_error(self, request_dict, response, operation, **kwargs):
        ...
        is_special_head_bucket = (
            error_code in ('301', '400') and operation.name == 'HeadBucket' and 'x-amz-bucket-region' in response_metadata.get('HTTPHeaders', {})
        )
        ...
        is_opt_in_region_redirect = error_code == 'IllegalLocationConstraintException' and operation.name != 'CreateBucket'
        if not any(
            [
...
class ArnParser:
    def parse_arn(self, arn):
        ...
        if len(arn_parts) < 6:
            raise InvalidArnException(f'Provided ARN: {arn} must be of the format: arn:partition:service:region:account:resource')
        return {
...
class S3EndpointSetter:
    ...
    def _validate_fips_supported(self, request):
        ...
        if 'outpost_name' in request.context['s3_accesspoint']:
            raise UnsupportedS3AccesspointConfigurationError(
                msg=(f'Client is configured to use the FIPS psuedo-region "{self._region}", but outpost ARNs do not support FIPS endpoints.')
            )
    ...
    def _validate_accesspoint_supported(self, request):
        if self._use_accelerate_endpoint:
            raise UnsupportedS3AccesspointConfigurationError(
                msg=('Client does not support s3 accelerate configuration when an access-point ARN is specified.')
            )
        ...
        if s3_service == 's3-object-lambda' and self._s3_config.get('use_dualstack_endpoint'):
            raise UnsupportedS3AccesspointConfigurationError(
                msg=('Client does not support s3 dualstack configuration when an S3 Object Lambda access point ARN is specified.')
            )
        ...
        if outpost_name and self._s3_config.get('use_dualstack_endpoint'):
            raise UnsupportedS3AccesspointConfigurationError(
                msg=('Client does not support s3 dualstack configuration when an outpost ARN is specified.')
            )
    ...
    def _validate_mrap_s3_config(self, request):
        ...
        if self._s3_config.get('s3_disable_multiregion_access_points'):
            ...
            raise UnsupportedS3AccesspointConfigurationError(
                msg=('Client does not support s3 dualstack configuration when a Multi-Region Access Point ARN is specified.')
            )
        ...
        if is_global_accesspoint(context):
            if HAS_CRT:
                ...
                raise MissingDependencyException(
                    msg='Using S3 with an MRAP arn requires an additional dependency. You will need to pip install botocore[crt] before proceeding.'
                )
...
class S3ControlArnParamHandler:
    ...
    def _override_account_id_param(self, params, arn_details):
        ...
        if 'AccountId' in params and params['AccountId'] != account_id:
            error_msg = ('Account ID in arn does not match the AccountId parameter provided: "{}"').format(params['AccountId'])
            raise UnsupportedS3ControlArnError(
...
class EventbridgeSignerSetter:
    ...
    def check_for_global_endpoint(self, params, context, **kwargs):
        ...
        if not HAS_CRT:
            raise MissingDependencyException(
                msg='Using EndpointId requires an additional dependency. You will need to pip install botocore[crt] before proceeding.'
            )

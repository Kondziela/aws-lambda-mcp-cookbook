...
def create_credential_resolver(session, cache=None, region_name=None):
    ...
    imds_config = {
        'ec2_metadata_service_endpoint': session.get_config_variable('ec2_metadata_service_endpoint'),
    ...
    assume_role_provider = AssumeRoleProvider(
        ...
        credential_sourcer=CanonicalNameCredentialSourcer([env_provider, container_provider, instance_metadata_provider]),
...
class RefreshableCredentials(Credentials):
    ...
    def _protected_refresh(self, is_mandatory):
        ...
        self._frozen_credentials = ReadOnlyCredentials(self._access_key, self._secret_key, self._token, self._account_id)
        if self._is_expired():
...
class ProcessProvider(CredentialProvider):
    ...
    def _retrieve_credentials_using(self, credential_process):
        ...
        if version != 1:
            raise CredentialRetrievalError(
                provider=self.METHOD,
                error_msg=(f"Unsupported version '{version}' for credential process provider, supported versions: 1"),
            )
        try:
            return {
...
class AssumeRoleProvider(CredentialProvider):
    ...
    def _get_role_config(self, profile_name):
        ...
        if credential_source is not None and source_profile is not None:
            raise InvalidConfigError(error_msg=(f'The profile "{profile_name}" contains both source_profile and credential_source.'))
    ...
    def _validate_credential_source(self, parent_profile, credential_source):
        if self._credential_sourcer is None:
            raise InvalidConfigError(
                error_msg=(
                    f'The credential_source "{credential_source}" is specified in profile "{parent_profile}", but no source provider was configured.'
                )
            )
        if not self._credential_sourcer.is_supported(credential_source):
            raise InvalidConfigError(
                error_msg=(f'The credential source "{credential_source}" referenced in profile "{parent_profile}" is not valid.')
            )
    ...
    def _validate_source_profile(self, parent_profile_name, source_profile_name):
        ...
        if source_profile_name not in profiles:
            raise InvalidConfigError(
                error_msg=(f'The source_profile "{source_profile_name}" referenced in the profile "{parent_profile_name}" does not exist.')
            )
    ...
    def _resolve_credentials_from_source(self, credential_source, profile_name):
        ...
        if credentials is None:
            raise CredentialRetrievalError(
                ...
                error_msg=(f'No credentials found in credential_source referenced in profile {profile_name}'),
            )
...
class SSOProvider(CredentialProvider):
    ...
    def _load_sso_config(self):
        ...
        if missing_config_vars:
            ...
            raise InvalidConfigError(
                error_msg=(f'The profile "{profile_name}" is configured to use SSO but is missing required configuration: {missing}')
            )
        return config

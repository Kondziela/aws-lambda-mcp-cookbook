...
class CSMSerializer:
    ...
    def _validate_client_id(self, csm_client_id):
        if len(csm_client_id) > self._MAX_CLIENT_ID_LENGTH:
            raise ValueError(
                f'The value provided for csm_client_id: {csm_client_id} exceeds the maximum length of {self._MAX_CLIENT_ID_LENGTH} characters'
            )
...
class SocketPublisher:
    ...
    def publish(self, event):
        ...
        if len(serialized_event) > self._MAX_MONITOR_EVENT_LENGTH:
            logger.debug(
                'Serialized event of size %s exceeds the maximum length allowed: %s. Not sending event to socket.',

    ...
    def _validate_attrs_are_nonzero(self):
        for attr, attr_val in self.__dict__.items():
            if attr_val is not None and attr_val <= 0:
                raise ValueError(f'Provided parameter {attr} of value {attr_val} must be greater than 0.')
...
class TransferManager:
    ...
    def __init__(self, client, config=None, osutil=None, executor_cls=None):
        ...
        self._request_executor = BoundedExecutor(
            ...
            tag_semaphores={
                ...
                IN_MEMORY_DOWNLOAD_TAG: SlidingWindowSemaphore(self._config.max_in_memory_download_chunks),
            },
    ...
    def _validate_all_known_args(self, actual, allowed):
        for kwarg in actual:
            if kwarg not in allowed:
                raise ValueError("Invalid extra_args key '{}', must be one of: {}".format(kwarg, ', '.join(allowed)))
    ...
    def _register_handlers(self):
        ...
        self._client.meta.events.register_last(event_name, signal_transferring, unique_id='s3upload-transferring')

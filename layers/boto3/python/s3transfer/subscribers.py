    ...
    def _validate_subscriber_methods(cls):
        for subscriber_type in cls.VALID_SUBSCRIBER_TYPES:
            ...
            if not callable(subscriber_method):
                raise InvalidSubscriberMethodError(f'Subscriber method {subscriber_method} must be callable.')

            if not accepts_kwargs(subscriber_method):
                raise InvalidSubscriberMethodError(f'Subscriber method {subscriber_method} must accept keyword arguments (**kwargs)')

...
class TransferCoordinator:
    ...
    def _transition_to_non_done_state(self, desired_state):
        with self._lock:
            if self.done():
                raise RuntimeError(f'Unable to transition from done state {self.status} to non-done state {desired_state}.')
    ...
    def submit(self, executor, task, tag=None):
        ...
        """
        logger.debug(f'Submitting task {task} to executor {executor} for transfer request: {self.transfer_id}.')

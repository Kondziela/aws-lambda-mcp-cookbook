    ...
    def _validate_if_bucket_supported(self, bucket):
        ...
        if self.VALIDATE_SUPPORTED_BUCKET_VALUES:
            for resource, pattern in self._UNSUPPORTED_BUCKET_PATTERNS.items():
                ...
                if match:
                    raise ValueError(f'TransferManager methods do not support {resource} resource. Use direct client calls instead.')

    def _validate_all_known_args(self, actual, allowed):
        for kwarg in actual:
            if kwarg not in allowed:
                raise ValueError(f"Invalid extra_args key '{kwarg}', must be one of: {', '.join(allowed)}")
...
class S3ClientArgsCreator:
    ...
    def _get_make_request_args_put_object(
        ...
        if not any(checksum_arg in call_args.extra_args for checksum_arg in FULL_OBJECT_CHECKSUM_ARGS):
    ...
    def _default_get_make_request_args(
        ...
        make_request_args = {
            ...
            'on_done': self.get_crt_callback(future, 'done', on_done_before_calls, on_done_after_calls),
        ...
        if make_request_args['type'] == S3RequestType.DEFAULT:
            make_request_args['operation_name'] = ''.join(x.title() for x in request_type.split('_'))

        arn_handler = _S3ArnParamHandler()
        if (accesspoint_arn_details := arn_handler.handle_arn(call_args.bucket)) and accesspoint_arn_details['region'] == '':

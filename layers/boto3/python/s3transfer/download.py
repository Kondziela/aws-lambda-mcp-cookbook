...
class GetObjectTask(Task):
        ...
        for i in range(max_attempts):
            try:
                ...
                if error_code == 'PreconditionFailed':
                    raise S3DownloadFailedError(f'Contents of stored object "{key}" in bucket "{bucket}" did not match expected ETag.')
                else:
                    raise
    ...
    def _validate_content_range(self, requested_range, content_range):
        ...
        if requested_range[6:] != response_range[6:]:
            raise S3ValidationError(f'Requested range: `{requested_range[6:]}` does not match content range in response: `{response_range[6:]}`')
